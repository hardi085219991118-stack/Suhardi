...
object FeatureRegistry {
  ...
  val features: List<FeatureContract> = listOf(
    ...
    FeatureContract(
      id = "FIRE-005",
      name = "Map Foundation",
      purpose = "Menampilkan peta menggunakan osmdroid dengan Esri World Imagery sebagai provider citra satelit utama.",
      input = "DeviceLocation terverifikasi dari FIRE-004",
      process = "Inisialisasi MapView dengan provider citra satelit Esri World Imagery (osmdroid), validasi viewport dan tile aktual, fallback ke OpenStreetMap sebagai peta referensi jika citra satelit gagal, rendering marker posisi pengguna jika valid, zero fire markers policy",
      output = "Layer peta citra satelit interaktif (Esri World Imagery) dengan fallback OpenStreetMap dan marker posisi GPS pengguna",
      dataSource = "Esri World Imagery",
